 ...
 *
 * A simplified fork of ICT-SMC that ONLY trades order block bounces.
 * Removes all M/W pattern detection "noise" that hurts performance.
 *
 * Key differences from ICT-SMC:
 * - No MWPatternDetector (removed entirely)
 * - Only generates OB_BOUNCE signals
 * - Keeps all structure analysis (CHoCH, MSS, Order Blocks)
 * - Keeps all entry/exit mechanics (LTF confirmation, stops, targets)
 *
 * Based on analysis showing:
 * - W_PATTERN: 818 trades, 27.75% WR, -$45,062 (84% of trades, 107% of losses)
 * - OB_BOUNCE: 76 trades, 40.79% WR, +$9,635 (profitable!)
 */
...
export class ICTOBStrategy extends BaseStrategy {
  constructor(params = {}) {
    super(params);
    ...
    this.params = {
      ...
      // Signal types to generate - ONLY OB_BOUNCE
      signalTypes: ['OB_BOUNCE'],
    ...
    // Pattern detector - used as a FILTER (reject entries when pattern is active)
    // This is the key difference from ICT-SMC: we INVERT the pattern logic
    this.patternDetector = new MWPatternDetector({
    ...
    // Track last pattern for filtering
  ...
  evaluateSignal(candle, prevCandle, marketData = {}, options = {}) {
    ...
    // Run pattern detection - used as a FILTER (reject entries when pattern is active)
    // This is the key difference from ICT-SMC: we only take trades when NO pattern is forming
    this.lastPattern = this.patternDetector.analyze(
      this.htfCandles,
      this.lastStructureAnalysis
    );

    // PATTERN FILTER: If a pattern is active (stage >= CHOCH), skip this candle
    // The 76 profitable OB_BOUNCE trades in ICT-SMC occurred specifically when no pattern was active
    if (this.lastPattern && this.lastPattern.stage >= MWPatternDetector.STAGES.CHOCH) {
      // If we were watching an OB for confirmation, cancel it - pattern context changed
      if (this.watchingOB) {
        this.clearWatchingOB();
      }
      return null;  // Skip - pattern is active, these become the losing W_PATTERN/M_PATTERN trades
    }
  ...
  checkLTFConfirmation(candle, candleIndex, currentTime) {
    ...
    if (confirmation.status === 'confirmed') {
      ...
      const entry = {
        ...
        trigger: 'ob_retest',
        signalType: 'OB_BOUNCE'  // Always OB_BOUNCE in this strategy
      };
  ...
  /**
   * Update candle aggregations
   * @param {Object} candle - New 1m candle
   */
  updateAggregations(candle) {
    const currentTime = this.toMs(candle.timestamp);

    // Update LTF (5m) candles
    const ltfPeriodStart = this.candleAggregator.getPeriodStart(
      currentTime,
      this.candleAggregator.getIntervalMinutes(this.params.entryTimeframe)
    );

    if (ltfPeriodStart > this.lastLTFTimestamp) {
      // New LTF period - aggregate
      this.ltfCandles = this.candleAggregator.aggregate(
        this.rawCandles,
        this.params.entryTimeframe
      );
      this.lastLTFTimestamp = ltfPeriodStart;
    }

    // Update HTF (4H) candles
    const htfPeriodStart = this.candleAggregator.getPeriodStart(
      currentTime,
      this.candleAggregator.getIntervalMinutes(this.params.structureTimeframe)
    );

    if (htfPeriodStart > this.lastHTFTimestamp) {
      // New HTF period - aggregate
      this.htfCandles = this.candleAggregator.aggregate(
        this.rawCandles,
        this.params.structureTimeframe
      );
      this.lastHTFTimestamp = htfPeriodStart;
    }
  }
  ...
  findEntry(currentCandle, currentTime) {
    ...
    // Use the entry finder with LTF candles - pass null for pattern (no patterns in this strategy)
    const entry = this.entryFinder.findEntry(
      this.ltfCandles,
      this.lastStructureAnalysis,
      null  // No pattern detection
    );

    if (entry) {
      // Always OB_BOUNCE in this strategy
      entry.signalType = this.determineSignalType(entry);
  ...
  /**
   * Determine the signal type based on entry
   * Simplified - always returns OB_BOUNCE for ob_retest triggers
   * @param {Object} entry
   * @returns {string}
   */
  determineSignalType(entry) {
    // Check entry trigger for OB_BOUNCE
    if (entry.trigger === 'ob_retest') {
      return 'OB_BOUNCE';
    }

    // Default to momentum continuation (shouldn't really happen with this strategy)
  ...
  getInfo() {
    return {
      name: 'ICT_OB',
      description: 'ICT Order Block Strategy (OB Bounce Only)',
