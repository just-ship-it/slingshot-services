#!/usr/bin/env node
/**
 * Skew-threshold sweep for iv-skew-gex on the corrected IV CSV.
 *
 * Holds the v6 balanced exit config constant (TP 120 / SL 80 / max-hold 60 /
 * breakeven 60→+5 / strong_negative regime block / level-prox 100) and varies
 * only --neg-skew-threshold (LONG) and --pos-skew-threshold (SHORT).
 *
 * The corrected IV distribution is asymmetric (mean +0.49%, stdev 1.21%, p10
 * -0.59%, p90 +1.80%), so the grid is asymmetric to match.
 *
 * Resumable: one JSON per combo, skipped if already present.
 *
 * Usage:
 *   node scripts/sweep-iv-skew-thresholds.js
 *   node scripts/sweep-iv-skew-thresholds.js --concurrency 3
 */
...
const RESULTS_DIR = path.join(BACKTEST_ROOT, 'data', 'sweep-results', 'iv-skew-thresholds-v7');

const GRID = {
  negSkew: [-0.003, -0.005, -0.0075, -0.01, -0.0125, -0.015],
  posSkew: [0.005, 0.0075, 0.01, 0.0125, 0.015, 0.02, 0.025],
};
...
async function main() {
  ...
  console.log(`\n=== iv-skew-gex skew-threshold sweep (v7 corrected IV) ===`);
  console.log(`Period: ${FIXED.start} → ${FIXED.end}`);
  console.log(`Fixed exits: TP=${FIXED.targetPoints} SL=${FIXED.stopLossPoints} maxHold=${FIXED.maxHoldBars} BE=${FIXED.breakevenTrigger}+${FIXED.breakevenOffset} prox=${FIXED.levelProximity}`);
    ...
    workers.push((async () => {
      while (true) {
        ...
        console.log(
          ...
          `${status.padEnd(8)} ${result.elapsedSec}s  ` +
          `${summary}  (elapsed ${elapsed}m, eta ${eta}m)`
        );
        if (result.code !== 0) {
...
function aggregate(combos) {
  ...
  const header = [
    'rank', 'negSkew', 'posSkew', 'trades', 'winRate', 'profitFactor',
    'sharpe', 'sortino', 'maxDrawdown', 'totalPnL', 'avgWin', 'avgLoss', 'expectancy',
  ];
  ...
  rows.forEach((r, i) => {
    lines.push([
      i + 1,
      r.negSkew, r.posSkew,
      r.trades, r.winRate, r.profitFactor,
      r.sharpe, r.sortino, r.maxDrawdown, r.totalPnL,
      r.avgWin, r.avgLoss, r.expectancy,
    ].join(','));
  });
  fs.writeFileSync(csvPath, lines.join('\n'));
  console.log(`\nAggregated results: ${csvPath}`);

  console.log(`\n=== Top 15 by PF (with target trade count 350-700, MaxDD < 8%) ===\n`);
  const filtered = rows.filter((r) =>
    (r.trades ?? 0) >= 350 && (r.trades ?? 0) <= 700 && (r.maxDrawdown ?? 100) < 8
  );
