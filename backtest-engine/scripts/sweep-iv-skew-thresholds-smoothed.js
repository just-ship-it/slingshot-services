#!/usr/bin/env node
/**
 * Skew-threshold sweep on the SMOOTHED IV CSV (5-min rolling median).
 *
 * Tests whether the strategy edge survives once minute-level noise is removed.
 * If a profitable threshold pair exists on smoothed IV, the v7 raw-IV results
 * were tuned to noise spikes — we have a true honest baseline at smoother
 * thresholds. If no combination is profitable, the strategy edge IS the noise
 * and won't replicate live.
 *
 * Smoothed distribution is tighter than raw (stdev 0.87% vs 1.21%, p10 -0.20%
 * vs -0.59%, p90 +1.28% vs +1.80%) so the threshold grid uses lower magnitudes.
 *
 * Usage:
 *   node scripts/sweep-iv-skew-thresholds-smoothed.js --concurrency 3
 */
...
const RESULTS_DIR = path.join(BACKTEST_ROOT, 'data', 'sweep-results', 'iv-skew-thresholds-v7-smoothed');

const GRID = {
  negSkew: [-0.001, -0.002, -0.003, -0.005, -0.0075, -0.01],
  posSkew: [0.003, 0.005, 0.0075, 0.01, 0.0125, 0.015],
};
...
async function main() {
  ...
  console.log(`\n=== iv-skew-gex skew-threshold sweep (v7 SMOOTHED IV) ===`);
  console.log(`Period: ${FIXED.start} → ${FIXED.end}`);
  console.log(`IV: ${FIXED.ivResolution} (5-min rolling median)`);
