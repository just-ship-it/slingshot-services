...
export class CLI {
  ...
  async parseArguments(argv) {
    return yargs(hideBin(argv))
      ...
      .option('slippage', {
        type: 'number',
        description: 'Market-order slippage in points per fill (entry/market/time exits). Default from config; set lower for liquid 1-lots (NQ spread = 0.25pt).',
        default: this.defaultConfig.backtesting.slippage.marketOrderSlippage
      })
      .option('stop-slippage', {
        type: 'number',
        description: 'Stop-order slippage in points per fill (stop_loss / trailing / BE exits). Default from config.',
        default: this.defaultConfig.backtesting.slippage.stopOrderSlippage
      })
      ...
      .option('lgpr-es-gate', {
        type: 'boolean',
        default: false,
        description: 'lt-gex-path-race: only take signals when the ES-15m clear-path composite agrees with the side (v1-ES sleeve; needs data/features/es15_clearpath_states.csv, ES data walls 2026-01)'
      })

      .option('lgpr-pullback', {
        type: 'number',
        description: 'lt-gex-path-race: limit-entry pullback as a fraction of the spot->stop range (default 0.10; deeper = higher RR, fewer fills)'
      })

      .option('lgpr-max-hold-min', {
        type: 'number',
        description: 'lt-gex-path-race: flat time-stop in minutes (default 480 = 8h)'
      })

      .option('lgpr-gex-max-age-min', {
        type: 'number',
        description: 'lt-gex-path-race: max GEX snapshot age in minutes at signal time (default 45)'
      })
  ...
  async executeBacktest(args) {
    ...
    if (!args.quiet) {
      ...
      console.log(chalk.white(`💧 Slippage: ${args.slippage}pt market / ${args['stop-slippage']}pt stop (per fill)`));
    ...
    // Generic JSON passthrough (applied first; specific flags below override)
    if (args['strategy-json']) {
      try {
        Object.assign(strategyParams, JSON.parse(args['strategy-json']));
      } catch (e) {
        throw new Error(`Invalid --strategy-json: ${e.message}`);
      }
    }
    ...
    const backtestConfig = {
      ...
      slippage: {
        limitOrderSlippage: this.defaultConfig.backtesting.slippage.limitOrderSlippage,
        marketOrderSlippage: args.slippage,
        stopOrderSlippage: args['stop-slippage']
      },
