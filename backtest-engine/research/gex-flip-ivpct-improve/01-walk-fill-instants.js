/**
 * Phase 1 — Walk each gex-flip-ivpct gold trade's fill instant forward in 1s OHLCV.
 *
 * Records per-bar [t_sec, hi, lo, c] signed favorable-positive PnL offsets so
 * the simulator can replay arbitrary exit policies (target/stop/BE/trail/maxHold/
 * fib-retrace/magnet-ratchet/market-aware) on the same fill instants.
 *
 * Gfi-specific fields carried through: ruleId (L1/L4/L3/S3/S1/S2), gexRegime,
 * ivPercentile, ivSkew, callWall/putWall/gammaFlip, hour/DOW.
 *
 * Walk window: from fillTs to MIN(fillTs + MAX_HOLD_MIN min, EOD 16:40 ET).
 *   - The gex-flip-ivpct gold uses --max-hold-bars implied 600 (10hr) but with
 *     entry window 04-13 ET and EOD 16:40 ET the practical ceiling is ~12.5hr.
 *     600min covers >99% of historical trades (max observed = 599min).
 *
...
const GS_PATH = arg('gold', path.join(DATA_DIR, 'gold-standard', 'gex-flip-ivpct-tight-s60t200be70.json'));
const OUT_PATH = arg('out', path.join(__dirname, 'output', '01-trades-walk.json'));
const MAX_HOLD_MIN = +arg('max_hold', '600');
const EMIT_STEP = +arg('emit_step', '0.25');
const EOD_ET_HH = +arg('eod_h', '16');
const EOD_ET_MM = +arg('eod_m', '40');
const MAX_HOLD_MS = MAX_HOLD_MIN * 60_000;

console.log(`\n=== Walk gfi fill instants (1s honest, per-bar OHLC) ===`);
console.log(`Gold standard: ${GS_PATH}`);
console.log(`Output: ${OUT_PATH}`);
console.log(`Max hold: ${MAX_HOLD_MIN}min  Emit step: ${EMIT_STEP}pt  EOD: ${EOD_ET_HH}:${String(EOD_ET_MM).padStart(2,'0')} ET\n`);
...
const events = trades.map((t, idx) => {
  ...
  return {
    ...
    sl: t.stopLoss,
    tp: t.takeProfit,
    contract: sig.signalContract || t.signalContract,
    ...
    terminal: null,
    // gfi-specific fields
    ruleId: sig.ruleId || null,
    rulePriority: sig.rulePriority ?? null,
    ruleStopPts: sig.stopPoints ?? null,
    ruleTargetPts: sig.targetPoints ?? null,
    gexRegime: sig.gexRegime ?? null,
    ivValue: sig.ivValue ?? null,
    ivSkew: sig.ivSkew ?? null,
    ivPercentile: sig.ivPercentile ?? null,
    gammaFlip: sig.gammaFlip ?? null,
    callWall: sig.callWall ?? null,
    putWall: sig.putWall ?? null,
    // distance to walls at entry (helps level-aware exit research)
    distCallWall: sig.callWall != null ? Math.abs(entry - sig.callWall) : null,
    distPutWall: sig.putWall != null ? Math.abs(entry - sig.putWall) : null,
    distGammaFlip: sig.gammaFlip != null ? Math.abs(entry - sig.gammaFlip) : null,
    // entry context
    hourEt: et.hour,
    minuteEt: et.minute,
    dow: et.dow,
...
function favSigned(e, h, l, c) {
  ...
  return [e.entry - l, e.entry - h, e.entry - c];
}
...
const slimmed = events.map(e => ({
  ...
  gexRegime: e.gexRegime,
  ivValue: e.ivValue,
  ivSkew: e.ivSkew,
  ivPercentile: e.ivPercentile,
  gammaFlip: e.gammaFlip,
  callWall: e.callWall,
  putWall: e.putWall,
  distCallWall: e.distCallWall,
  distPutWall: e.distPutWall,
  distGammaFlip: e.distGammaFlip,
  hourEt: e.hourEt,
  minuteEt: e.minuteEt,
  dow: e.dow,
