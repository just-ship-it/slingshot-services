/**
 * Phase 4c — Extension sweep: wider targets (240-320), stop=50 vs 60,
 * BE 120-180, refining around the cartesian best.
 */
import fs from 'fs';
...
const TARGETS = [220, 240, 260, 280, 300, 320];
const STOPS = [50, 60];
const BE_TRIGS = [null, 100, 120, 140, 160, 180];
const BE_OFFS = [5, 10, 20];
...
top(valid, 'sharpe', false, 'Sharpe', 20);
top(valid, 'pf', false, 'PF', 20);

const out = './output/04c-sweep-extend.csv';
const header = 'target,stop,beTrig,beOff,maxHoldMin,n,pnl,wr,pf,sharpe,maxDD,worstLoss';
const lines = [header, ...rows.sort((a,b) => b.pnl - a.pnl).map(r => [r.target, r.stop, r.beTrig, r.beOff, r.maxHoldMin, r.n, r.pnl.toFixed(0), r.wr.toFixed(2), r.pf.toFixed(3), r.sharpe.toFixed(3), r.maxDD.toFixed(0), r.worstLoss.toFixed(0)].join(','))];
