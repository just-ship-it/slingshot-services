/**
 * Phase 1 — Walk gex-level-fade gold trades from fill_ts forward in 1s OHLCV.
 *
 * Adapted from research/gex-lt-3m-improve/01-walk-fill-instants.js.
 *
 * For each completed gold trade, stream 1s NQ OHLCV from the trade's
 * actualEntry timestamp forward to MIN(fillTs + MAX_HOLD_MIN, EOD 15:45 ET),
 * recording per-bar [t_sec, hi_fav, lo_fav, c_fav] favorable-positive PnL
 * offsets from entry. The simulator (02-sim-exits.js) replays arbitrary exit
 * policies on this data.
 *
 * Per-trade metadata carried through: levelType, side, hourEt, dow, episodeNum,
 * gold exit reason / pnl / mfe / mae / duration.
 *
 * Emit policy: bar emitted iff (hi|lo|c) changed by >= EMIT_STEP since the
 * previous emit, or it's the first/final bar.
 */
...
const GS_PATH = arg('gold', path.join(DATA_DIR, 'gold-standard', 'gex-level-fade.json'));
const OUT_PATH = arg('out', path.join(__dirname, 'output', '01-trades-walk.json'));
const MAX_HOLD_MIN = +arg('max_hold', '240');   // 4 hours — wide headroom; gold maxHold is 180
const EMIT_STEP = +arg('emit_step', '0.25');
const EOD_ET_HH = 15;
const EOD_ET_MM = 45;
const MAX_HOLD_MS = MAX_HOLD_MIN * 60_000;

console.log(`\n=== Walk gex-level-fade fill instants (1s honest) ===`);
...
const events = trades.map((t, idx) => {
  ...
  return {
    ...
    finalTs: null,
    terminal: null,
    levelType: t.signal?.levelType || null,
    levelPrice: t.signal?.levelPrice ?? null,
    episodeNum: t.signal?.episodeNum ?? null,
    hourEt: et.hour,
    minuteEt: et.minute,
    dow: et.dow,
...
function favSigned(e, h, l, c) {
  ...
  return [e.entry - l, e.entry - h, e.entry - c];   // hi=fav-extreme, lo=adv-extreme
}
