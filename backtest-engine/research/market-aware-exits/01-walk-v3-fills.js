/**
 * Phase 1 — Walk v3's 553 trades in 1s OHLCV.
 *
 * Identical to gex-lt-3m-improve/01-walk-fill-instants.js but sourced from the
 * v3 engine output (553 trades) instead of the W12 gold-standard (888). Walk
 * captures per-1s-bar [t_sec, hi, lo, c] favorable-positive PnL offsets from
 * fill instant forward to min(150min, EOD 15:45 ET).
 *
 * Output: output/01-trades-walk-v3.json (~80MB).
 */
...
const SRC_PATH = arg('src', path.resolve(__dirname, '../gex-lt-3m-improve/output/engine-v3.json'));
const OUT_PATH = arg('out', path.join(__dirname, 'output', '01-trades-walk-v3.json'));
...
console.log(`\n=== Walk v3 fill instants (1s honest, per-bar OHLC) ===`);
...
const trades = src.trades.filter(t => t.status === 'completed' && t.actualEntry != null && t.entryTime != null);
...
const events = trades.map((t, idx) => {
  ...
  return {
    ...
    minuteEt: et.minute,
    dow: et.dow,
    v3ExitReason: t.exitReason,
    v3NetPnL: t.netPnL,
    v3PointsPnL: t.pointsPnL,
    v3MfePoints: t.mfePoints,
    v3MaePoints: t.maePoints,
    v3DurationMs: t.duration,
    v3ActualExit: t.actualExit,
  };
...
const slimmed = events.map(e => ({
  ...
  minuteEt: e.minuteEt,
  dow: e.dow,
  v3ExitReason: e.v3ExitReason,
  v3NetPnL: e.v3NetPnL,
  v3PointsPnL: e.v3PointsPnL,
  v3MfePoints: e.v3MfePoints,
  v3MaePoints: e.v3MaePoints,
  v3DurationMs: e.v3DurationMs,
  v3ActualExit: e.v3ActualExit,
  walk: e.walk,
}));
