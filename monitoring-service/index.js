...
// Short-DTE IV strategy status - proxied from signal-generator (multi-strategy engine)
app.get('/api/strategy/short-dte-iv/status', dashboardAuth, async (req, res) => {
  try {
    const response = await axios.get(`${SIGNAL_GENERATOR_URL}/strategy/status/short-dte-iv`, { timeout: 5000 });
    res.json({ success: true, ...response.data });
  } catch (error) {
    if (error.code === 'ECONNREFUSED') {
      res.json({
      ...
      logger.error('Failed to fetch short-dte-iv status:', error.message);
      res.status(500).json({ error: 'Failed to fetch short-dte-iv status', details: error.message });
    }
  }
});

// LT Candle Regime strategy status - proxied from signal-generator
app.get('/api/strategy/lt-candle-regime/status', dashboardAuth, async (req, res) => {
  try {
    const response = await axios.get(`${SIGNAL_GENERATOR_URL}/strategy/status/lt-candle-regime`, { timeout: 5000 });
    res.json({ success: true, ...response.data });
  } catch (error) {
    if (error.code === 'ECONNREFUSED') {
      res.json({
      ...
      logger.error('Failed to fetch lt-candle-regime status:', error.message);
      res.status(500).json({ error: 'Failed to fetch lt-candle-regime status', details: error.message });
    }
  }
});

// GEX-FLIP-IVPCT strategy status - proxied from signal-generator
app.get('/api/strategy/gex-flip-ivpct/status', dashboardAuth, async (req, res) => {
  try {
    const response = await axios.get(`${SIGNAL_GENERATOR_URL}/strategy/status/gex-flip-ivpct`, { timeout: 5000 });
    res.json({ success: true, ...response.data });
  } catch (error) {
    if (error.code === 'ECONNREFUSED') {
      res.json({
      ...
      logger.error('Failed to fetch gex-flip-ivpct status:', error.message);
      res.status(500).json({ error: 'Failed to fetch gex-flip-ivpct status', details: error.message });
    }
  }
});
...
app.get('/api/strategy/gex-level-fade/status', dashboardAuth, async (req, res) => {
  try {
    const response = await axios.get(`${SIGNAL_GENERATOR_URL}/strategy/status/gex-level-fade`, { timeout: 5000 });
    res.json({ success: true, ...response.data });
  } catch (error) {
    if (error.code === 'ECONNREFUSED') {
      res.json({
      ...
      logger.error('Failed to fetch gex-level-fade status:', error.message);
      res.status(500).json({ error: 'Failed to fetch gex-level-fade status', details: error.message });
    }
  }
});
